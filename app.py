...
DATA = ROOT / "data"
CLEANED_DB_FILE = DATA / "CLEANED_ACADEMIC_DATABASE.xlsx"
POLICY_FILE = DATA / "policy_documents.csv"
ALIASES_FILE = DATA / "course_aliases.csv"
STUDENT_PROFILE_FILE = DATA / "synthetic_student_profiles.csv"
STUDENT_HISTORY_FILE = DATA / "student_course_history.csv"
MINOR_SOURCE_NAME = "Minor_Courses_for_BTech_Students(3).xlsx"
STRUCT_SOURCE_NAME = "Semester_Spread_Structures_Sept_2026(3).xlsx"
...
def load_authoritative_data():
    ...
    """
    structured_file = CLEANED_DB_FILE
    ...
    minor["source_workbook"] = MINOR_SOURCE_NAME
    ...
    sem["source_workbook"] = STRUCT_SOURCE_NAME
    ...
    structures["source_workbook"] = STRUCT_SOURCE_NAME
    ...
    req["source_workbook"] = STRUCT_SOURCE_NAME
    ...
    for _,r in sem[sem.course_title.astype(str).str.strip().ne("")].iterrows():
        course_parts.append({"course_code":r.course_code,"course_title":r.course_title,"credits":r.credits,
            ...
            "source_workbook":STRUCT_SOURCE_NAME,"semester":r.semester,"category":r.category,"basket":r.basket})
    for _,r in minor[minor.course_title.astype(str).str.strip().ne("")].iterrows():
        course_parts.append({"course_code":r.course_code,"course_title":r.course_title,"credits":r.credits,
            ...
            "source_workbook":MINOR_SOURCE_NAME,"semester":r.semester,"category":"","basket":""})
    ...
    for _,r in course_master[course_master.course_title.astype(str).str.strip().ne("")].iterrows():
        course_parts.append({"course_code":r.course_code,"course_title":r.course_title,"credits":"",
            ...
            "source_workbook":STRUCT_SOURCE_NAME,"semester":"","category":"","basket":""})
...
def synthetic_students():
    """Load reproducible synthetic profiles and history from deployment data files."""
    profiles=pd.read_csv(STUDENT_PROFILE_FILE).fillna("") if STUDENT_PROFILE_FILE.exists() else pd.DataFrame()
    if profiles.empty:
        profiles=pd.DataFrame([
            ["SYN001","BTech","2026",5,72,"Finance"],["SYN002","BTech","2026",6,96,"Psychology"],
            ["SYN003","BTech","2025",7,120,"Finance"],["SYN004","BTech","2026",5,72,"Finance"],
            ["SYN005","BTech","2025",7,118,"Marketing"],["SYN006","BTech","2026",6,94,"Economics"]
        ],columns=["student_id","programme","batch","current_semester","total_credits","minor"])
    profiles=profiles.rename(columns={"current_semester":"semester","total_credits":"completed_credits"})
    keep=["student_id","programme","batch","semester","completed_credits","minor"]
    for c in keep:
        if c not in profiles.columns: profiles[c]=""
    profiles=profiles[keep].copy()
    if STUDENT_HISTORY_FILE.exists():
        history=pd.read_csv(STUDENT_HISTORY_FILE).fillna("")
    else:
        history=pd.DataFrame(columns=["student_id","course_code","status","grade"])
    for c in ["student_id","course_code","status","grade"]:
        if c not in history.columns: history[c]=""
    return profiles, history[["student_id","course_code","status","grade"]].copy()
...
def build_rag():
    ...
    for _,r in minors.iterrows():
        records.append((record_to_text(r.to_dict()),{"record_type":"minor_course","source":"authoritative_excel","source_workbook":MINOR_SOURCE_NAME,"source_sheet":r.get("source_sheet","") ,"source_row":r.get("source_row","")}))
    for _,r in semesters.iterrows():
        records.append((record_to_text(r.to_dict()),{"record_type":"semester_course","source":"authoritative_excel","source_workbook":STRUCT_SOURCE_NAME,"source_sheet":r.get("source_sheet","") ,"source_row":r.get("source_row","")}))
    for _,r in structures.iterrows():
        records.append((record_to_text(r.to_dict()),{"record_type":"programme_structure","source":"authoritative_excel","source_workbook":STRUCT_SOURCE_NAME,"source_sheet":r.get("source_sheet","") ,"source_row":r.get("source_row","")}))

    # Add the normalized relationship/requirement tables as first-class retrieval records.
    # This prevents semantic retrieval from depending only on the wide source-table rows.
    extra_specs = [
        ("prerequisites", "prerequisite"),
        ("course_relationships", "course_relationship"),
        ("degree_requirements", "degree_requirement"),
        ("semester_offerings", "semester_offering"),
        ("basket_requirements", "basket_requirement"),
        ("course_master", "course_master"),
    ]
    for sheet_name, record_type in extra_specs:
        try:
            extra = pd.read_excel(CLEANED_DB_FILE, sheet_name=sheet_name).fillna("")
            for _,r in extra.iterrows():
                records.append((record_to_text(r.to_dict()), {
                    "record_type": record_type,
                    "source": "cleaned_academic_database",
                    "source_workbook": "CLEANED_ACADEMIC_DATABASE.xlsx",
                    "source_sheet": sheet_name,
                    "source_row": r.get("source_row", r.name + 2 if hasattr(r, "name") else ""),
                }))
        except Exception:
            pass
...
def direct_answer(query, student_id, rag, plan=None):
    ...
    if "credit requirements for different structures" in ql or ("different structures" in ql and "credit" in ql):
        allsum=[]
        for _,rr in requirements.iterrows():
            struct_name=norm(rr.get("source_sheet", ""))
            basket=norm(rr.get("basket", ""))
            credits=norm(rr.get("credits", ""))
            if struct_name and basket and credits:
                allsum.append(f"{struct_name}: {basket} — {credits} credits")
        return {"answer":"Credit requirements recorded across the supplied structures:\n\n"+"\n".join(f"- {x}" for x in allsum),"status":"verified","evidence":[{"source":"degree_requirements","row":"","workbook":"Semester & Structure workbook","text":x} for x in allsum]}
