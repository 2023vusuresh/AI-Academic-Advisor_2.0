...
def find_course(query, courses):
    ...
    # Treat parenthetical acronyms as part of the same canonical title.
    # Example: "Financial and Management Accounting" ↔
    # "Financial and Management Accounting (FAMA)".
    if len(exact_title)==0:
        stripped_titles=title_norm.map(lambda t: re.sub(r"\b(?:[a-z]{2,8})\b$", "", t).strip())
        exact_title=courses[stripped_titles.eq(qt)]

    if len(exact_title)==0:
        embedded=courses[title_norm.map(lambda t: bool(t) and (t in qt or qt in t))]
        if len(embedded)==0:
            embedded=courses[stripped_titles.map(lambda t: bool(t) and (t in qt or qt in t))]
...
def _deterministic_structured_response(base):
    """Create the deterministic response contract used when Gemini is unavailable."""
    status=base.get("status","verified")
    answer=str(base.get("answer","")).strip()
    ...
    if status=="greeting":
        return {
            "headline":"AIRA",
            "direct_answer":answer or "How can I help you with the academic records?",
            "details":[],
            "note":""
        }
    ...
    if status=="follow_up":
        return {
            ...
            "details":[],
            "note":"The clarification is required to select the correct academic record."
        }

    if status=="insufficient":
        return {
            "headline":"Information not specified",
            "direct_answer":answer,
            "details":[],
            "note":"The answer is limited to the supplied academic sources."
        }

    if status=="conflict":
        return {
            "headline":"Conflicting academic records",
            "direct_answer":answer,
            "details":[],
            "note":"AIRA does not select between conflicting source records without the applicable academic scope."
        }
    ...
    return {
        ...
        "direct_answer":answer,
        "details":[],
        "note":"Based only on the supplied academic sources."
    }
...
def gemini_structured_response(query, base, student_id=None):
    """Generate the student-facing wording from an already verified result."""
    status=base.get("status","verified")
    evidence=base.get("evidence",[])
    evidence_text="\n".join(
        f"- {e.get('text','')} | workbook={e.get('workbook','')} | "
        f"sheet={e.get('source','')} | row={e.get('row','')}"
        for e in evidence
    )

    schema={
        "headline":"short heading",
        "direct_answer":"direct answer preserving the verified result",
        "details":["only verified details from the result"],
        "note":"scope or uncertainty note if needed"
    }

    prompt=f"""You are AIRA, a university academic advisor.

The VERIFIED_RESULT below has already been computed from the authoritative academic knowledge base.
Your task is ONLY to present that verified result clearly.

Return ONLY valid JSON matching this schema:
{json.dumps(schema, ensure_ascii=False)}

NON-NEGOTIABLE RULES:
1. VERIFIED_RESULT is the factual source of truth.
2. Do not add, infer, correct, or invent any academic fact.
3. Preserve course codes, course names, credits, prerequisites, batches, semesters, counts and policy statements exactly.
4. Do not combine different academic batches, structures or source versions unless VERIFIED_RESULT explicitly did so.
5. Never convert an unknown or unrecorded field into a definite value.
6. For status=follow_up, direct_answer MUST preserve the specific clarification contained in VERIFIED_RESULT. Do not replace it with a generic request such as "provide more information".
7. For status=insufficient, direct_answer MUST preserve the specific missing-information statement contained in VERIFIED_RESULT.
8. For status=conflict, report the conflict exactly as represented in VERIFIED_RESULT and do not choose one record.
9. details may contain only facts already present in VERIFIED_RESULT or SOURCE_EVIDENCE.
10. Do not mention Gemini, LangChain, LangGraph, RAG, prompts, models, tools or internal processing.
11. Do not create a separate explanatory subsection; keep the response direct and academically precise.
12. Keep the response concise and academically precise.
...
"""
    ...
    if not isinstance(obj,dict):
        return _render_structured_response(_deterministic_structured_response(base)), err or "No structured response returned"

    for key in ["headline","direct_answer","note"]:
        obj.setdefault(key,"")
    obj.setdefault("details",[])
    if not isinstance(obj.get("details"),list):
        obj["details"]=[]

    # For uncertainty states, the deterministic clarification is authoritative.
    if status in {"follow_up","insufficient","conflict"}:
        obj["direct_answer"]=base.get("answer","")

    rendered=_render_structured_response(obj)
    if not rendered:
        rendered=_render_structured_response(_deterministic_structured_response(base))
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Academic facts are grounded in the supplied university source package.")
