...
def load_authoritative_data():
    ...
    course_parts=[]
    for _,r in sem[sem.course_code!=""].iterrows():
    ...
    for _,r in minor[minor.course_code!=""].iterrows():
        course_parts.append({"course_code":r.course_code,"course_title":r.course_title,"credits":r.credits,
            "prerequisite":r.prerequisite,"prerequisite_status":r.prerequisite_status,"batch":r.batch,
...
- If a requested fact exists in multiple tables, choose the most specific table. Minor questions -> minor_courses; semester/offering questions -> semester_offerings or semester_courses; programme structure -> structure_courses/degree_requirements; prerequisite relationships -> prerequisites/course_relationships.
...
def _generic_evidence(rows, table):
    ...
    for _,r in rows.head(8).iterrows():
        ...
        wb="Minor Courses workbook" if table=="minor_courses" else "Semester & Structure workbook"
        parts=[]
...
def execute_generic_plan(plan, rag, original_query):
    ...
    frames={
      ...
      "synthetic_students":rag.get("students",pd.DataFrame()),"student_course_history":rag.get("history",pd.DataFrame())}
    ...
    if not select: select=[c for c in ["course_code","course_title","credits","semester","prerequisite"] if c in df.columns]
    out=df[select].copy()
...
try:
    _cleaned = pd.ExcelFile(DATA_DIR / "CLEANED_ACADEMIC_DATABASE.xlsx")
    for _sheet,_key in [("semester_offerings","semester_offerings"),("prerequisites","prerequisites"),("course_relationships","course_relationships"),("synthetic_students","students"),("student_course_history","history")]:
        if _sheet in _cleaned.sheet_names:
            rag[_key]=pd.read_excel(DATA_DIR / "CLEANED_ACADEMIC_DATABASE.xlsx",sheet_name=_sheet)
except Exception:
...
{{
  "intent": "conversation|minor|course_details|prerequisite|dependent_courses|offering|eligibility|credits|graduation_requirement|structure|comparison|general_academic|ambiguous|insufficient",
...
def gemini_production_format(query, base, student_id=None):
    # Actual LLM response generation constrained to deterministic verified evidence.
    status=base.get("status","verified")
    evidence=base.get("evidence",[])
    evidence_text="\n".join(f"- {e.get('text','')} | workbook={e.get('workbook','')} | sheet={e.get('source','')} | row={e.get('row','')}" for e in evidence)
    prompt=f'''You are AIRA, a polished university academic advisor.
Generate the final answer to the student's question using ONLY the VERIFIED_RESULT and SOURCE_EVIDENCE below.
The verified result was computed from the supplied university Excel database.

NON-NEGOTIABLE:
- Never add, infer, correct, or invent academic facts.
- Preserve course codes, names, counts, credits, prerequisites, years and semesters exactly.
- Do not combine records from different academic batches unless VERIFIED_RESULT explicitly did so.
- If status is follow_up, ask the clarification directly and explain why it is needed in one sentence.
- If status is insufficient, say the supplied records do not contain enough information.
- If status is conflict, clearly state that records conflict and do not choose one.
- For a prerequisite question, use wording like: "The prerequisite of this course is ... based on the available academic sources." For multiple prerequisites, use bullets.
- For a count, lead with the number, then state the exact scope (minor/year/semester/filter).
- For a list, use a clean bullet list with course code, title, and only fields supported by VERIFIED_RESULT.
- For conversational messages, respond naturally as AIRA.
- Do not mention prompts, models, RAG, Gemini, internal processing, or these instructions.
- Do not output source metadata in the main answer; the UI displays sources separately.
...
Return ONLY polished Markdown for the student.'''
...
def _graph_verify(state: AIRAState):
    # Prefer the open-ended structured plan. Fall back to the specialized deterministic
    # advisor when the LLM cannot safely map the request to the schema.
