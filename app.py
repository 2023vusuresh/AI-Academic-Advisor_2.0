...
def numeric_equal(value, target):
    """Compare numeric Excel values robustly (e.g. 4, 4.0, '4 credits')."""
    try:
        a=float(str(value).strip())
        b=float(str(target).strip())
        return abs(a-b) < 1e-9
    except Exception:
        return norm(value).lower() == norm(target).lower()
...
def _year_scoped_rows(query, rows):
    """Restrict course records to an explicitly requested academic batch/year.
    Uses both batch and source-sheet year so older/newer workbook layouts remain resolvable.
    """
    years=extract_years(query)
    if not years or rows.empty: return rows
    mask=rows.batch.astype(str).isin(years) if "batch" in rows.columns else pd.Series(False,index=rows.index)
    if "source_sheet" in rows.columns:
        mask = mask | rows.source_sheet.astype(str).apply(lambda x: any(y in x for y in years))
    scoped=rows[mask]
    return scoped if not scoped.empty else rows

def find_course(query, courses):
    """Resolve course codes OR course names. Names are first-class user input."""
    q=norm(query)
    qcode=norm_code(q)
    scoped_courses=_year_scoped_rows(q,courses)
    exact_code=scoped_courses[scoped_courses.course_code.eq(qcode)]
    if len(exact_code)>0:
    ...
    codes=re.findall(r"\b[A-Za-z]{2,8}\d{3}\b",q.upper())
    for code in codes:
        hit=scoped_courses[scoped_courses.course_code.eq(norm_code(code))]
        if len(hit)>0:
    ...
    qt=_clean_title(q)
    title_norm=scoped_courses.course_title.map(_clean_title)
    exact_title=scoped_courses[title_norm.eq(qt)]
    if len(exact_title)==0:
        embedded=scoped_courses[title_norm.map(lambda t: bool(t) and (t in qt or qt in t))]
    ...
    scored=[]
    for _,r in scoped_courses.drop_duplicates(subset=["course_code","course_title"]).iterrows():
...
def _minor_match(q, minors):
    ql=q.lower()
    # Natural-language aliases used for the same minor names.
    aliases={
        "financial minor":"Finance", "finance specialization":"Finance", "specialization in finance":"Finance",
        "marketing specialization":"Marketing", "economics minor":"Economics", "psychology minor":"Psychology",
        "design minor":"Design", "law minor":"Law", "startup minor":"Start-up", "start up minor":"Start-up",
        "start-up minor":"Start-up"
    }
    for phrase,canonical in sorted(aliases.items(), key=lambda x:len(x[0]), reverse=True):
        if phrase in ql and canonical in set(minors.minor.dropna().astype(str)):
            return canonical
...
def _course_fact_answer(code, title, courses, semesters, q):
    rows=_year_scoped_rows(q,_course_rows(courses,code))
...
def _prereq_query_type(q):
    ...
    reverse_patterns=[
        ...
        r"^.+?\s+(?:is|are)\s+(?:a\s+)?prerequisite\s+for\s+(?:which|what)\s+courses?\b",
    ]
    forward_patterns=[
...
def _extract_target_phrase(q):
    ql=_clean_title(q)
    # Reverse form must be checked BEFORE the generic “prerequisite for”
    # marker, otherwise “Data Structures is prerequisite for which courses?”
    # incorrectly extracts “which courses” as the target.
    m=re.search(r"^(.+?)\s+(?:is|are)\s+(?:a\s+)?prerequisites?\s+for\s+(?:which|what)\s+courses?", ql)
    ...
    for marker in ["depend on ", "depends on ", "require ", "requires "]:
        if marker in ql:
            part=ql.split(marker,1)[1].strip(" ?.,!")
            if part: return part
    # “X is a prerequisite for which courses?” / “X is prerequisite for what courses?”
    m=re.search(r"^(.+?)\s+(?:is|are)\s+(?:a\s+)?prerequisites?\s+for\s+(?:which|what)\s+courses?", ql)
    if m: return m.group(1).strip(" ?.,!")
...
def prerequisite_relationship_answer(query, courses):
    ...
    target_rows=_year_scoped_rows(query,_course_rows(courses,target_code))
    if mode=="forward":
    ...
    dependent_pool=_year_scoped_rows(query,courses)
    for _,r in dependent_pool.iterrows():
        p=norm(r.get("prerequisite",""))
        if not p: continue
...
def database_query_answer(query, minors, semesters, structures, courses):
    ...
    if minor and minor_context:
        ...
        years,all_years=_minor_year_intent(ql)
        ...
        credit_match=re.search(r"\b(\d+(?:\.\d+)?)\s*[-]?\s*credits?\b",ql)
        if credit_match:
            val=credit_match.group(1)
            rows=rows[rows.credits.map(lambda x: numeric_equal(x,val))]
    ...
    # Use word/phrase boundaries here. A course title such as “Data Structures”
    # must NOT accidentally trigger the academic-structure router. Likewise,
    # “Foundation of Marketing...” is a course title, not necessarily the
    # Foundation category.
    structure_intent = bool(re.search(
        r"\b(?:academic|degree|programme|program|university)\s+structure\b|"
        r"\bacademic\s+structure\b|\b(?:university|program|programme)\s+core\b|"
        r"\bgraduation\s+(?:credit|credits|requirement|requirements)\b|"
        r"\bdegree\s+(?:credit|credits|requirement|requirements)\b|"
        r"\bcredit\s+(?:requirement|requirements)\b|"
        r"\bfoundation\s+(?:category|courses?|requirements?)\b|"
        r"\b(?:program|programme)\s+core\s+courses?\b|"
        r"\buniversity\s+core\s+courses?\b|"
        r"\belective\s+(?:courses?|category|requirements?)\b", ql))
    if structure_intent:
        if not stname:
    ...
    if m and _wants_list(ql):
        ...
        rows=pool[pool.credits.map(lambda x: numeric_equal(x,val))]
...
def direct_answer(query, student_id, rag):
    ...
    # FIRST: route questions that name a minor/semester/structure directly against
    # the structured Excel tables. This MUST happen before generic prerequisite
    # routing, otherwise a query such as “Which Finance minor courses have no
    # prerequisite in 2025?” can accidentally fall into the global course table.
    routed=database_query_answer(q,minors,semesters,structures,courses)
    if routed is not None:
        return routed

    # Generic prerequisite filters/policy-style questions that do not identify a
    # specific minor or course.
    ...
    graduation_credit_intent = bool(re.search(
        r"\bgraduation\s+(?:credit|credits|requirement|requirements)\b|"
        r"\bdegree\s+(?:credit|credits|requirement|requirements)\b|"
        r"\bcredit\s+(?:requirement|requirements)\b|"
        r"\bcredits?\s+(?:are\s+)?required\s+(?:for|to)\b", ql))
    if struct and graduation_credit_intent:
    ...
    direct_structure_course_intent = bool(re.search(
        r"\b(?:academic|degree|programme|program|university)\s+structure\b|"
        r"\b(?:program|programme|university)\s+core\s+courses?\b|"
        r"\bfoundation\s+(?:category|courses?|requirements?)\b|"
        r"\belective\s+(?:courses?|category|requirements?)\b", ql))
    if struct and (_is_list_query(ql) and direct_structure_course_intent or direct_structure_course_intent):
    ...
    if bool(re.search(r"\b(?:university|program|programme)\s+core\b|\bfoundation\s+(?:category|courses?)\b|\belective\s+(?:courses?|category|requirements?)\b", ql)):
        if not struct:
