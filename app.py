...
DATA = ROOT / "data"
SOURCE_ORIGINALS = DATA / "source_originals"
# Raw Excel workbooks are retained only for provenance/reproducibility.
# Runtime retrieval uses CLEANED_ACADEMIC_DATABASE.xlsx plus policy_documents.csv.
MINOR_FILE = SOURCE_ORIGINALS / "Minor_Courses_for_BTech_Students(3).xlsx"
STRUCT_FILE = SOURCE_ORIGINALS / "Semester_Spread_Structures_Sept_2026(3).xlsx"
...
def load_authoritative_data():
    """Load the cleaned/canonical runtime database produced from the supplied sources.

    The application does NOT retrieve directly from raw Excel layouts. The cleaned workbook
    is the runtime structured source-of-truth for tabular facts; the Handbook/SOP page-level
    records are loaded from policy_documents.csv for policy retrieval. Original source files
    are retained only for provenance/reproducibility.
    """
...
def friendly_source(e):
    ...
    row = e.get("row", "")
    page = e.get("page", "")
    is_pdf = str(e.get("workbook", "")).lower().endswith(".pdf") or str(e.get("source_file", "")).lower().endswith(".pdf") or "Handbook" in raw or "SOP" in raw
    if is_pdf and page not in ("", None, "nan", "None"):
        return f"{label}  ·  {raw}  ·  Page {page}"
...
def _graph_verify(state: AIRAState):
    # Verification order matters. Specific course/prerequisite questions must be
    # resolved against the canonical tables BEFORE a broad semantic plan is allowed
    # to return an empty result. This prevents a valid title-only record (for example
    # Financial and Management Accounting (FAMA), whose source code is unknown) from
    # being incorrectly reported as "not found".
    ...
    if base is None:
        # Deterministic source verification is authoritative for identifiable
        # course/minor/structure/student questions. It also handles ambiguity and
        # missing prerequisite data with a specific follow-up/insufficient state.
        base = direct_answer(state["original_query"], state.get("student_id") or None, rag, plan=state.get("det_plan"))
    if base is None:
        # Only genuinely open-ended questions reach the generic semantic executor.
