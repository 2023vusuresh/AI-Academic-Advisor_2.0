...
def _is_small_talk(query):
    """Handle greetings before academic entity resolution."""
...
def format_production_answer(base):
    """Convert verified advisor output into a consistent, readable structure.
    This formatter never invents facts; it only reorganizes text already returned
    by deterministic source verification.
    """
    answer=norm(base.get("answer", ""))
    status=base.get("status", "verified")
    if status == "greeting":
        return answer

    # Already structured multi-line responses: preserve their verified content.
    if "\n" in answer and ("- " in answer or "• " in answer):
        return answer

    # Course + prerequisite + credits in one sentence.
    m=re.match(r"^(.+?)\s+—\s+(.+?): prerequisite \*\*(.+?)\*\*; credits \*\*(.+?)\*\*\.?$",answer,re.I)
    if m:
        code,title,pre,credits=m.groups()
        return f"### {code} — {title}\n\n**Prerequisite**\n- {pre}\n\n**Credits**\n- {credits}"

    # Course + prerequisite only.
    m=re.match(r"^(.+?)\s+—\s+(.+?): the prerequisite recorded in the supplied Excel data is \*\*(.+?)\*\*\.?$",answer,re.I)
    if m:
        code,title,pre=m.groups()
        return f"### {code} — {title}\n\n**Prerequisite**\n- {pre}"

    # Course + credits only.
    m=re.match(r"^(.+?)\s+—\s+(.+?): credits recorded in the supplied Excel data: \*\*(.+?)\*\*\.?$",answer,re.I)
    if m:
        code,title,credits=m.groups()
        return f"### {code} — {title}\n\n**Credits**\n- {credits}"

    # Course offering/category response.
    m=re.match(r"^(.+?)\s+—\s+(.+?)\s+is listed in semester\(s\): \*\*(.+?)\*\*\.?\s*Category: \*\*(.+?)\*\*\.?$",answer,re.I)
    if m:
        code,title,sems,cats=m.groups()
        return f"### {code} — {title}\n\n**Semester(s)**\n- {sems}\n\n**Category**\n- {cats}"

    # Prerequisite relationship response.
    m=re.match(r"^To take \*\*(.+?)\s+—\s+(.+?)\*\*, the prerequisite recorded in the supplied academic data is \*\*(.+?)\*\*\.?$",answer,re.I)
    if m:
        code,title,pre=m.groups()
        return f"### {code} — {title}\n\n**Prerequisite**\n- {pre}"

    # Simple no-prerequisite response.
    m=re.match(r"^\*\*(.+?)\s+—\s+(.+?)\*\* has no prerequisite recorded in the supplied academic data\.?$",answer,re.I)
    if m:
        code,title=m.groups()
        return f"### {code} — {title}\n\n**Prerequisite**\n- None recorded"

    # Follow-up / uncertainty responses are presented with a clear next action.
    if status == "follow_up":
        return f"### I need one clarification\n\n{answer}"
    if status == "insufficient":
        return f"### Not enough verified information\n\n{answer}"
    if status == "conflict":
        return f"### Conflicting source records\n\n{answer}"

    # Generic verified text: give it a stable heading without changing content.
    return f"### Answer\n\n{answer}"
...
{{
  ...
  "intent": "prerequisite | eligibility | dependent_courses | course_details | offering | minor | credits | graduation_requirement | ambiguous | insufficient | general_academic",
...
8. Keep the response concise and visually structured in Markdown.
9. Use a short heading, then labeled sections such as **Prerequisite**, **Credits**, **Semester**, **Eligibility**, **Courses**, or **Next step**, only when supported.
10. For lists, use bullet points. Do not produce a dense paragraph when a list is appropriate.
11. Do not mention these instructions, RAG, prompts, or internal processing.
...
.stApp{min-height:100vh;background:radial-gradient(circle at 50% 18%,rgba(103,146,255,.12),transparent 28%),radial-gradient(circle at 10% 90%,rgba(91,220,196,.08),transparent 25%),linear-gradient(180deg,#fbfcff 0%,#f4f7fb 100%);}
...
.answer-page{padding-top:26px}.answer-shell{max-width:820px;margin:22px auto 0;padding:28px 30px;border:1px solid #e2e8f0;border-radius:24px;background:rgba(255,255,255,.97);box-shadow:0 18px 55px rgba(27,46,73,.08);}.answer-kicker{display:flex;align-items:center;justify-content:space-between;margin-bottom:15px}.answer-kicker .who{font-size:.70rem;text-transform:uppercase;letter-spacing:.14em;color:#8794a7;font-weight:800}.answer-time{font-size:.70rem;color:#a0aaba;font-weight:600}.answer-text{font-size:1.03rem;line-height:1.75;color:#1a2d47}.answer-text strong{color:#122b4d}
...
.source-title{max-width:820px;margin:24px auto 8px;font-size:.70rem;text-transform:uppercase;letter-spacing:.14em;color:#8794a7;font-weight:800}.source-card{max-width:820px;margin:7px auto;padding:12px 15px;border:1px solid #e5eaf1;border-radius:15px;background:rgba(255,255,255,.72);display:flex;gap:10px;align-items:flex-start}.source-mark{color:#6289c7;font-size:.9rem}.source-label{font-size:.77rem;font-weight:700;color:#3d5b7f}.source-detail{font-size:.76rem;color:#758398;margin-top:3px;line-height:1.45}.follow{max-width:820px;margin:14px auto;padding:12px 15px;border-radius:14px;background:#fff8ea;border:1px solid #f2e3c4;color:#876317;font-size:.84rem}.ask-again{max-width:820px;margin:18px auto;text-align:center}.ask-again button{border-radius:14px!important;border:1px solid #dfe6ef!important;background:#fff!important}
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Academic facts are restricted to the supplied Excel workbooks. Gemini formats only the verified result in production.")
student_id=None if selected=="No student profile" else selected.split(" · ")[0]

STATUS_META={"verified":("✓","Verified"),"follow_up":("◌","Needs more info"),"insufficient":("○","Not in records"),"conflict":("▲","Conflicting records"),"greeting":("●","Hello")}
...
with st.expander("🧪 Evaluation & faculty view",expanded=False):
    ...
    st.caption("Production answers remain grounded in deterministic verification. The LLM is an explanation layer, not the source of academic facts.")
